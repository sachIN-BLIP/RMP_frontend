      ...
      <div className="mx-12 pb-10 flex flex-col-reverse md:flex-row justify-between">
        <div className="departments rounded-xl bg-neutral-100 md:w-[530px]  w-[90%] mx-auto p-6 md:p-10 md:mr-20 md:mx-0 self-start">
          <div className="button flex bg-white p-3  items-center justify-between rounded-lg " onClick={() => {setDropShow(!dropShow);setEffect(true)}}>
            ...
            <button className="text-gray-900 text-opacity-70  text-base sm:text-lg font-normal leading-6">Select Department</button>
          ...
          <div className={dropShow? `dropdown bg-white rounded-t-lg mt-1 divide-y divide-slate-200  overflow-auto max-h-[235px] `
                      : "hidden"}>
            
            {allDepartments.map((dept) => {return (<div className="option p-[10px] text-gray-900  text-[16px] font-normal leading-loose border-b-1 border-gray-400 pl-2">
              {dept}
            </div>) }    )}
